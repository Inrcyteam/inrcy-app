...
test("le durcissement SQL ne réintroduit plus le seuil historique de 39 Mio", () => {
  const sql = read(
  ...
  assert.doesNotMatch(sql, /40894464/);
  assert.doesNotMatch(verify, /40894464/);
  assert.match(sql, /313524224/);
  assert.match(verify, /313524224/);
});
