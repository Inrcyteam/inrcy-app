                ...
                <div style={{ display: "grid", gap: 7, alignSelf: "end" }}>
                  ...
                  {(item.onRemove || item.onRemoveEverywhere) ? (
                    <div style={{ display: "grid", gridTemplateColumns: item.onRemove && item.onRemoveEverywhere ? "1fr 1fr" : "1fr", gap: 6 }}>
                      {item.onRemove ? (
                        <button type="button" className={buttonClassName} onClick={item.onRemove} style={{ justifyContent: "center", fontSize: 12, padding: "0 8px" }}>Retirer</button>
                      ) : null}
      ...
      <div onClick={(event) => event.stopPropagation()} style={{ width: modalWidth, maxWidth: "100%", height: modalHeight, maxHeight: "100%", minWidth: 0, minHeight: 0, alignSelf: isMobile ? "stretch" : undefined, justifySelf: isMobile ? "stretch" : undefined, borderRadius: isMobile ? 20 : 28, border: "1px solid rgba(255,255,255,0.12)", background: "linear-gradient(180deg, rgba(24,28,42,0.985), rgba(14,17,28,0.985))", boxShadow: "0 28px 100px rgba(0,0,0,0.5)", padding: modalPadding, display: "grid", gridTemplateRows: "auto minmax(0, 1fr)", gap: isMobile ? 10 : 16, overflow: "hidden", boxSizing: "border-box" }}>
        <div style={{ display: isMobile ? "grid" : "flex", alignItems: isMobile ? "start" : "center", justifyContent: "space-between", gap: isMobile ? 8 : 12, minHeight: isMobile ? "auto" : 52, flexWrap: "wrap", minWidth: 0 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: isMobile ? 6 : 8, flexShrink: 1, flexWrap: isMobile ? "nowrap" : "wrap", justifyContent: "flex-end", width: isMobile ? "100%" : undefined, minWidth: 0 }}>
            {onApplyToChannelImages ? <button type="button" className={buttonClassName} onClick={onApplyToChannelImages} style={{ minWidth: isMobile ? 128 : 168, minHeight: isMobile ? 42 : 44, height: isMobile ? 42 : 44, flex: isMobile ? "1 1 128px" : undefined, justifyContent: "center", alignItems: "center", fontSize: isMobile ? 11 : undefined, padding: isMobile ? "0 12px" : "0 16px" }}>Appliquer aux images</button> : null}
            {onApplyToSelectedChannels ? <button type="button" className={buttonClassName} onClick={onApplyToSelectedChannels} style={{ minWidth: 0, minHeight: isMobile ? 42 : 44, height: isMobile ? 42 : 44, flex: isMobile ? "1 1 auto" : undefined, justifyContent: "center", alignItems: "center", fontSize: isMobile ? 11 : undefined, padding: isMobile ? "0 8px" : "0 16px" }}>Appliquer aux canaux</button> : null}
            {onResetChannel ? <button type="button" className={buttonClassName} onClick={onResetChannel} style={{ minWidth: isMobile ? 112 : 132, minHeight: isMobile ? 42 : 44, height: isMobile ? 42 : 44, flex: isMobile ? "1 1 112px" : undefined, justifyContent: "center", alignItems: "center", fontSize: isMobile ? 11 : undefined, padding: isMobile ? "0 12px" : "0 16px" }}>Réinit. canal</button> : null}
