...
export default function ThanksModal({ styles, onClose, trackEvent }: { styles: typeof stylesDash; onClose: () => void; trackEvent: (type: "thanks_mail", payload: Record<string, any>) => Promise<void> }) {
...
useEffect(() => {
  ...
  const onSend = async () => {
    ...
    setSaving(true);
    try {
      await trackEvent("thanks_mail", { recipients });
      onClose();
    } finally {
      setSaving(false);
    }
