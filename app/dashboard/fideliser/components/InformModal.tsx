...
export default function InformModal({ styles, onClose, trackEvent }: { styles: typeof stylesDash; onClose: () => void; trackEvent: (type: "newsletter_mail", payload: Record<string, any>) => Promise<void> }) {
...
useEffect(() => {
  ...
  const onSend = async () => {
    ...
    setSaving(true);
    try {
      await trackEvent("newsletter_mail", { recipients });
      onClose();
    } finally {
      setSaving(false);
    }
