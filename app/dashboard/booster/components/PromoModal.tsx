...
export default function PromoModal({ styles, onClose, trackEvent }: { styles: typeof stylesDash; onClose: () => void; trackEvent: (type: "promo_mail", payload: Record<string, any>) => Promise<void> }) {
...
useEffect(() => {
  ...
  const onSend = async () => {
    ...
    setSaving(true);
    try {
      await trackEvent("promo_mail", { recipients });
      onClose();
    } finally {
      setSaving(false);
    }
