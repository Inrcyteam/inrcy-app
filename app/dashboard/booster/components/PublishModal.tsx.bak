...
export default function PublishModal({
  ...
  const [finalReviewPosts, setFinalReviewPosts] = useState<Partial<Record<ChannelKey, ChannelPost>> | null>(null);
  ...
  const [channelDetails, setChannelDetails] = useState<Record<ChannelKey, ChannelConnectionDetail>>(EMPTY_CHANNEL_DETAILS);
  const [channelInfoOpen, setChannelInfoOpen] = useState<ChannelKey | null>(null);
  ...
  const clearGenerationTimers = () => {
    generationTimersRef.current.forEach((timerId) => window.clearTimeout(timerId));
  ...
  const imageAdapterChannels = useMemo<ChannelKey[]>(() => {
    ...
    if (selectedChannels.includes("inrcy_site")) adapterChannels.push("inrcy_site");
    ...
    if (selectedChannels.includes("instagram")) adapterChannels.push("instagram");
  ...
  const getImpactedImageChannels = (channel: ChannelKey): ChannelKey[] => [channel];
  ...
  const onGenerate = async () => {
    ...
    generationTimersRef.current = generationSteps.map((step) => window.setTimeout(() => {
  ...
  const onImagesChange = async (files: FileList | null, targetChannel?: ChannelKey) => {
    ...
    if (targetChannel) {
      setChannelImageEditors((prev) => {
        ...
        next[targetChannel] = {
          imageKeys: targetChannel === "gmb"
            ? ((current.imageKeys || []).length ? [...(current.imageKeys || [])] : newKeys.slice(0, 1))
  ...
  const getPreviewCtaForDisplayKey = (key: DisplayKey, post: ChannelPost) => {
    ...
    if (mode === "none") return "";
    if (mode === "call") {
      const label = explicit || getChannelDefaultCtaLabel(key, "call") || "Appeler";
      return phone ? `${label} · ${phone}` : label;
    }
    if (explicit) return explicit;
    if (mode === "website") return getChannelDefaultCtaLabel(key, mode);
    if (mode === "message") return key === "instagram" ? "Message privé" : "Envoyer un message";
    return "";
  };
  ...
  const getPublicationPreviewForChannel = (channel: ChannelKey) => {
    ...
    const transform = firstImageKey
      ? editor.transforms?.[firstImageKey] || getOptimizedTransform(channel, imageMetaByKey[firstImageKey])
      : undefined;
    ...
    return {
      ...
      hashtags: displayKey === "instagram" ? getLiveInstagramHashtags() : post.hashtags || [],
      imageCount: selectedKeys.length,
      formatLabel: channel === "inrcy_site" || channel === "site_web"
        ? "Rendu site / iframe"
      ...
      images: selectedKeys.map((imageKey) => ({
        ...
        transform: editor.transforms?.[imageKey] || getOptimizedTransform(channel, imageMetaByKey[imageKey]),
        preset: CHANNEL_PRESETS[channel],
  ...
  const toggleChannelImage = (channel: ChannelKey, imageKey: string) => {
    ...
    setActiveImageKeyByChannel((prev) => {
      ...
      return {
        ...prev,
        ...Object.fromEntries(impactedChannels.map((targetChannel) => [targetChannel, nextKeys[0] || ""])),
      };
    });
  };
  ...
  const resetActiveChannelImages = () => {
    const imageKeysForChannel = channelImageEditors[activeImageChannel]?.imageKeys || [];
    if (!imageKeysForChannel.length) return;
    setChannelImageEditors((prev) => {
      const next = { ...prev };
      const current = next[activeImageChannel] || { imageKeys: imageKeysForChannel, transforms: {} };
      const transforms = { ...current.transforms };
      for (const imageKey of imageKeysForChannel) {
        transforms[imageKey] = getOptimizedTransform(activeImageChannel, imageMetaByKey[imageKey]);
      }
      next[activeImageChannel] = { ...current, imageKeys: imageKeysForChannel, transforms };
      return next;
    });
  };
  ...
  const moveChannelImage = (channel: ChannelKey, imageKey: string, direction: -1 | 1) => {
    setChannelImageEditors((prev) => {
      const current = prev[channel] || { imageKeys: imageKeys.slice(), transforms: {} };
      const index = current.imageKeys.indexOf(imageKey);
      const targetIndex = index + direction;
      if (index < 0 || targetIndex < 0 || targetIndex >= current.imageKeys.length) return prev;
  ...
  const getReviewPostForChannel = (channel: ChannelKey, preparedPostsByChannel: Partial<Record<ChannelKey, ChannelPost>>) => {
  ...
  const buildFinalReviewItems = (preparedPostsByChannel: Partial<Record<ChannelKey, ChannelPost>>) => {
    return selectedChannels.map((channel) => {
      ...
      if (!imageKeysToPublish.length) {
        if (channel === "instagram") blockers.push("Instagram nécessite au moins 1 image.");
        else if (channel === "gmb") warnings.push("Google Business sera publié sans photo.");
  ...
  const finalReviewItems = finalReviewOpen ? buildFinalReviewItems(finalReviewPosts || buildPreparedPostsByChannel()) : [];
  ...
  const finalReviewSiteNotice = selectedChannels.includes("inrcy_site") && selectedChannels.includes("site_web")
    ? getPublishImageKeysForChannel("inrcy_site").join("|") !== getPublishImageKeysForChannel("site_web").join("|")
    : false;

  const publishReadinessItems = buildFinalReviewItems(buildPreparedPostsByChannel());
  const imageAdapterTabs = imageAdapterChannels.map((channel) => {
    const reviewItem = publishReadinessItems.find((item) => item.channel === channel);
    const blockerCount = reviewItem?.blockers.length || 0;
    const warningCount = reviewItem?.warnings.length || 0;
    const count = reviewItem?.imageCount ?? getPublishImageKeysForChannel(channel).length;
    return {
      key: channel,
      label: getImageAdapterLabel(channel),
      count,
      tone: blockerCount ? "blocked" as const : warningCount ? "warning" as const : count ? "ready" as const : "empty" as const,
    };
  });
  ...
  const confirmFinalReview = async () => {
    const preparedPostsByChannel = finalReviewPosts || buildPreparedPostsByChannel();
  ...
  return (
    <div style={{ display: "grid", gap: 12, minWidth: 0 }}>
      ...
      {finalReviewOpen ? (
        <div style={{ position: "fixed", inset: 0, zIndex: 10012, background: "rgba(4, 8, 18, 0.74)", backdropFilter: "blur(8px)", display: "grid", placeItems: "center", padding: 16, overflowY: "auto", overscrollBehavior: "contain" }}>
          <div className={styles.blockCard} style={{ width: "min(760px, 100%)", maxHeight: "calc(100vh - 32px)", overflowY: "auto", display: "grid", gap: 16, background: "#111827", backgroundImage: "none", border: "1px solid rgba(148, 163, 184, 0.28)", boxShadow: "0 30px 90px rgba(0,0,0,0.62)", backdropFilter: "none" }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "flex-start", flexWrap: "wrap" }}>
              <div style={{ display: "grid", gap: 6 }}>
                <div style={{ fontSize: 22 }}>✅</div>
                <div className={styles.blockTitle} style={{ marginBottom: 0 }}>Vérification avant publication</div>
                <div style={{ fontSize: 13, color: "rgba(255,255,255,0.72)", lineHeight: 1.5 }}>
                  Contrôlez les canaux, les images et les alertes avant l’envoi final.
                </div>
              </div>
              <div style={{ fontSize: 12, padding: "7px 10px", borderRadius: 999, background: "rgba(76,195,255,0.10)", border: "1px solid rgba(76,195,255,0.22)", color: "rgba(255,255,255,0.86)" }}>
                {finalReviewItems.length} canal(aux) sélectionné(s)
              </div>
            </div>

            {finalReviewSiteNotice ? (
              <div style={{ borderRadius: 14, padding: 12, background: "rgba(76,195,255,0.08)", border: "1px solid rgba(76,195,255,0.18)", color: "rgba(255,255,255,0.82)", fontSize: 13, lineHeight: 1.5 }}>
                Site iNrCy et Site web ont des images ou un ordre différent : c’est normal, les deux canaux sont indépendants.
              </div>
            ) : null}
            ...
            <div style={{ display: "grid", gap: 10 }}>
              {finalReviewItems.map((item) => {
                ...
                return (
                  <div key={item.channel} style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "minmax(140px, 0.8fr) minmax(120px, 0.55fr) minmax(0, 1.4fr)", gap: 10, alignItems: "center", borderRadius: 16, padding: 12, background: "rgba(255,255,255,0.04)", border: item.blockers.length ? "1px solid rgba(248,113,113,0.34)" : hasAlerts ? "1px solid rgba(251,191,36,0.26)" : "1px solid rgba(255,255,255,0.08)" }}>
                    <div style={{ minWidth: 0, display: "grid", gap: 5 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                        <div style={{ fontWeight: 900, color: "#fff" }}>{item.label}</div>
                        <span style={{ fontSize: 10, fontWeight: 900, padding: "4px 7px", borderRadius: 999, background: item.blockers.length ? "rgba(248,113,113,0.14)" : item.warnings.length ? "rgba(251,191,36,0.14)" : "rgba(34,197,94,0.14)", color: item.blockers.length ? "#fecaca" : item.warnings.length ? "#fde68a" : "#bbf7d0", border: item.blockers.length ? "1px solid rgba(248,113,113,0.25)" : item.warnings.length ? "1px solid rgba(251,191,36,0.25)" : "1px solid rgba(34,197,94,0.25)" }}>
                          {item.blockers.length ? "Bloquant" : item.warnings.length ? "À vérifier" : "Prêt"}
                        </span>
                      </div>
                      <div style={{ fontSize: 12, color: "rgba(255,255,255,0.58)" }}>Canal sélectionné</div>
                    </div>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      <span style={{ fontSize: 12, padding: "6px 9px", borderRadius: 999, background: "rgba(255,255,255,0.07)", color: "rgba(255,255,255,0.84)" }}>
                        {item.imageCount ? `${item.imageCount} image${item.imageCount > 1 ? "s" : ""}` : "Aucune image"}
                      </span>
                      <span style={{ fontSize: 12, padding: "6px 9px", borderRadius: 999, background: item.hasContent ? "rgba(34,197,94,0.12)" : "rgba(251,191,36,0.12)", color: item.hasContent ? "#bbf7d0" : "#fde68a" }}>
                        {item.hasContent ? "Texte OK" : "Texte vide"}
                      </span>
                    </div>
                    <div style={{ display: "grid", gap: 6, fontSize: 12, lineHeight: 1.45 }}>
                      {!hasAlerts ? (
                        <span style={{ color: "#bbf7d0" }}>Prêt à publier.</span>
                      ) : null}
                      {item.warnings.map((warning) => (
                        <span key={warning} style={{ color: "#fde68a" }}>⚠️ {warning}</span>
                      ))}
                      {item.blockers.map((blocker) => (
                        <span key={blocker} style={{ color: "#fecaca" }}>⛔ {blocker}</span>
                      ))}
                    </div>
                  </div>
                );
            ...
            {hasFinalReviewBlockers ? (
              <div style={{ borderRadius: 14, padding: 12, background: "rgba(248,113,113,0.10)", border: "1px solid rgba(248,113,113,0.24)", color: "#fecaca", fontSize: 13, lineHeight: 1.5 }}>
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, flexWrap: "wrap", position: "sticky", bottom: -1, paddingTop: 4, background: "#111827" }}>
              <button type="button" className={styles.secondaryBtn} onClick={closeFinalReview}>Retour modifier</button>
              <button type="button" className={styles.primaryBtn} onClick={confirmFinalReview} disabled={hasFinalReviewBlockers || saving} style={{ opacity: hasFinalReviewBlockers || saving ? 0.58 : 1 }}>
                {saving ? "Publication en cours..." : "Confirmer la publication"}
              </button>
            </div>
          </div>
        </div>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  position: "relative",
                  overflow: "visible",
                  borderColor: isSelected ? "rgba(76,195,255,0.45)" : "rgba(255,255,255,0.10)",
                  boxShadow: isSelected ? "0 0 0 1px rgba(76,195,255,0.18) inset, 0 10px 24px rgba(8,18,34,0.18)" : "none",
                  background: isSelected ? "rgba(76,195,255,0.08)" : "rgba(255,255,255,0.03)",
              ...
              >
                <span style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0, flex: 1 }}>
                  <input
                    ...
                    style={{ width: 18, height: 18, accentColor: "#4cc3ff", cursor: isConnected ? "pointer" : "not-allowed", flexShrink: 0 }}
                  />
                  <span style={{ width: 10, height: 10, borderRadius: 999, background: isConnected ? "#43d17d" : "#ff6b7d", boxShadow: isConnected ? "0 0 12px rgba(67,209,125,0.35)" : "0 0 12px rgba(255,107,125,0.25)", flexShrink: 0 }} />
                  <span style={{ minWidth: 0, flex: 1, display: "flex", alignItems: "center", gap: 8, textAlign: "left", overflow: "hidden" }}>
                    <span style={{ minWidth: 0, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", fontWeight: 800, flexShrink: 0 }}>
                      {CHANNEL_LABELS[key]}
                    </span>
                    {!isMobile && info ? (
                      <>
                        <span style={{ width: 4, height: 4, borderRadius: 999, background: "rgba(255,255,255,0.3)", flexShrink: 0 }} />
                        <span style={{ minWidth: 0, fontSize: 12, lineHeight: 1.35, color: "rgba(255,255,255,0.68)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                          {info.desktopLabel}
                        </span>
                      </>
                    ) : null}
                ...
                {info ? (
                  <>
                    <button
                      ...
                      title={isMobile ? `Voir les détails de ${CHANNEL_LABELS[key]}` : info.fullLabel}
                      onPointerDown={(event) => event.stopPropagation()}
                      onClick={(event) => {
                        event.stopPropagation();
                        setChannelInfoOpen((prev) => (prev === key ? null : key));
                      }}
                      style={{
                        width: 32,
                        height: 32,
                        ...
                        background: isInfoVisible ? "rgba(76,195,255,0.14)" : "rgba(255,255,255,0.06)",
                        ...
                        flexShrink: 0,
                        cursor: "pointer",
                        boxShadow: isInfoVisible ? "0 0 0 1px rgba(76,195,255,0.16) inset" : "none",
                      }}
                    >
                      🔗
                    </button>
                    {isInfoVisible ? (
                      ...
                      >
                        <div style={{ fontSize: 12, lineHeight: 1.35, color: "rgba(255,255,255,0.92)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{info.mobileLabel}</div>
                      </div>
                    ) : null}
                  </>
                ) : null}
      ...
      <div className={styles.blockCard} style={{ minWidth: 0, maxWidth: "100%", boxSizing: "border-box" }}>
        ...
        <div style={{ display: "grid", gap: 10 }}>
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "minmax(0, 1fr) minmax(0, 1fr)", gap: 10, alignItems: "start" }}>
            <div>
          ...
          <div style={{ display: "grid", gap: 6, justifyItems: "start" }}>
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              <button type="button" className={styles.primaryBtn} onClick={onGenerate} disabled={generating}>
                {generating ? `${generationStage || "Génération"} ${generationProgress}%` : "Générer avec iNrCy"}
              </button>
              <button type="button" className={styles.secondaryBtn} onClick={onReset}>Réinitialiser</button>
            </div>
            {generating ? <div style={{ fontSize: 12, color: "rgba(255,255,255,0.72)" }}>iNrCy prépare les variantes adaptées à chaque canal.</div> : null}
          </div>
        </div>
      </div>
...
<div className={styles.subtitle} style={{ marginBottom: 10, maxWidth: "none", whiteSpace: "normal" }}>
              ...
              <div style={{ display: "grid", gap: 10 }}>
                ...
                <div>
                  {(() => {
                    ...
                    const updateTarget = activeCard === "site" ? "site_web" : activeCard;
                      ...
                      <>
                        <div style={{ display: "grid", gridTemplateColumns: ctaGridColumns, gap: 10, alignItems: "start" }}>
                          ...
                          {ctaMode === "website" ? (
                            <>
                              <div>
                                <div style={{ fontSize: 12, opacity: 0.85, marginBottom: 6 }}>Lien du CTA</div>
                                <input
                                  value={currentPost.ctaUrl || ""}
                                  onChange={(e) => updatePost(updateTarget, { ctaUrl: e.target.value })}
                              ...
                              <div>
                                <div style={{ fontSize: 12, opacity: 0.85, marginBottom: 6 }}>Libellé du lien</div>
                                <input
                                  value={currentPost.cta}
                                  onChange={(e) => updatePost(updateTarget, { cta: e.target.value })}
                                  style={lightFieldStyle}
                          ...
                          {ctaMode === "call" ? (
                            <div>
                              <div style={{ fontSize: 12, opacity: 0.85, marginBottom: 6 }}>Téléphone</div>
                              <input
                                value={currentPost.ctaPhone || ""}
                                onChange={(e) => updatePost(updateTarget, { ctaPhone: e.target.value })}
                                style={lightFieldStyle}
                          ...
                          {ctaMode === "custom" ? (
                            <div>
                              <div style={{ fontSize: 12, opacity: 0.85, marginBottom: 6 }}>Libellé du CTA</div>
                              <input
                                value={currentPost.cta}
                                onChange={(e) => updatePost(updateTarget, { cta: e.target.value })}
                                style={lightFieldStyle}
                        ...
                        {ctaMode === "website" && ctaDefaults?.preferredWebsiteUrl ? (
                        ...
                        {(ctaMode === "website" || ctaMode === "custom") ? renderLimitCounter("CTA", currentPost.cta.length, CHANNEL_TEXT_GUIDELINES[activeCard].cta) : null}
                      </>
                    );
                  })()}
                </div>
      ...
      <div className={styles.blockCard} style={{ minWidth: 0, maxWidth: "100%", boxSizing: "border-box" }}>
        <div className={styles.blockTitle} style={{ marginBottom: 8 }}>Images de la publication</div>
        <div className={styles.subtitle} style={{ marginBottom: 12, maxWidth: "none", whiteSpace: "normal" }}>
          Ajoutez vos images, puis gérez directement leur rendu par canal. Les images sont cochées par défaut sur tous les canaux, sauf Google Business limité à une photo.
        </div>
        <input
          ref={fileInputRef}
          type="file"
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center", marginBottom: 12 }}>
          ...
          {images.length ? <div style={{ fontSize: 12, opacity: 0.85 }}>{images.length} image(s) ajoutée(s)</div> : <div style={{ fontSize: 12, opacity: 0.7 }}>Aucune image ajoutée</div>}
        </div>
        {imgError ? <div style={{ marginBottom: 10, fontSize: 13, color: "#ffb4b4" }}>{imgError}</div> : null}
        {!selectedChannels.length ? (
          <div style={{ fontSize: 13, opacity: 0.75 }}>Sélectionnez d’abord vos canaux.</div>
        ) : !images.length ? (
          <div style={{ fontSize: 13, opacity: 0.75 }}>Ajoutez une ou plusieurs images. Elles apparaîtront directement dans les onglets des canaux.</div>
        ) : (
          <>
            {activeImageChannel === "gmb" ? (
              <div style={{ marginBottom: 12, borderRadius: 14, padding: "12px 14px", border: "1px solid rgba(251,191,36,0.26)", background: "rgba(251,191,36,0.10)", display: "grid", gap: 10 }}>
                <div style={{ fontSize: 13, lineHeight: 1.5, color: "#fde68a" }}>
                  <strong>Google Business : 1 seule photo par publication.</strong> Les autres images restent disponibles sur les autres canaux.
                </div>
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            ...
            <ChannelImageAdapterCardsPanel
              ...
              items={imageKeys.map((key, index) => {
                ...
                const usedChannelCount = selectedChannels.filter((channel) => (channelImageEditors[channel]?.imageKeys || []).includes(key)).length;
                ...
                return {
                  ...
                  onMovePrevious: included && selectedKeysForActiveChannel.indexOf(key) > 0 ? () => moveChannelImage(activeImageChannel, key, -1) : undefined,
                  onMoveNext: included && selectedKeysForActiveChannel.indexOf(key) >= 0 && selectedKeysForActiveChannel.indexOf(key) < selectedKeysForActiveChannel.length - 1 ? () => moveChannelImage(activeImageChannel, key, 1) : undefined,
                };
              })}
      ...
      <ChannelImageAdapterModal
        ...
        onApplyToChannelImages={(channelImageEditors[activeImageChannel]?.imageKeys || []).length > 1 ? applyCurrentCadrageToActiveChannelImages : undefined}
        onResetChannel={(channelImageEditors[activeImageChannel]?.imageKeys || []).length ? resetActiveChannelImages : undefined}
        ...
        onApplyToSelectedChannels={activeImageChannel === "inrcy_site" || activeImageChannel === "site_web" ? undefined : applyCurrentImageToSelectedChannels}
      ...
      <div ref={publishAreaRef} style={{ display: "grid", gap: 8, justifyItems: "end", scrollMarginBottom: 24 }}>
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", flexWrap: "wrap", alignItems: "center" }}>
          ...
          <button
            ...
            onClick={onPublish}
            disabled={saving}
            style={{ minHeight: 52, padding: "0 24px", fontSize: 16, fontWeight: 800, opacity: saving ? 0.64 : 1, cursor: saving ? "wait" : "pointer" }}
          >
            {saving ? `Publication en cours ${publishProgress}%` : "Vérifier et publier"}
          </button>
        </div>
