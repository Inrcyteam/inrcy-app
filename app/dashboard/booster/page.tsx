...
export default function BoosterPage() {
  ...
  const [publishEditorOverlayOpen, setPublishEditorOverlayOpen] =
    useState(false);
  const [publishHasUnsavedChanges, setPublishHasUnsavedChanges] =
    useState(false);
      ...
      try {
        if (type === "publish") {
          ...
          const failed = Object.entries(
            (json?.results || {}) as Record<string, any>,
          ).filter(([, value]) => value && value.ok === false);
          if (summary?.allFailed || (!summary && failed.length)) {
            const detail = failed
              .map(
                ([channel, value]) =>
                  `${channel}: ${String((value as any)?.error || "erreur")}`,
              )
              .join(" | ");
            throw new Error(
              getSimpleFrenchErrorMessage(detail, "La publication a échoué."),
            );
          }
  ...
  const data = useMemo(() => {
    ...
    const featureMissionDone = Boolean(
      weeklySummary?.missions?.weeklyFeatureUse?.done,
    );
    const createActuDone = Boolean(weeklySummary?.missions?.createActu?.done);
    const missionProjected = Number(
      weeklySummary?.missions?.weeklyFeatureUse?.projected ??
        Math.round(10 * turbo),
    );
    const actuProjected = Number(
      weeklySummary?.missions?.createActu?.projected ?? Math.round(10 * turbo),
    );
    const featureGained = Number(
      weeklySummary?.missions?.weeklyFeatureUse?.gained ?? 0,
    );
    ...
    const buildMissionHelper = (
      doneThisCard: number,
      missionDone: boolean,
      fallback: string,
    ) => {
      if (missionDone && doneThisCard <= 0)
        return "Cette action reste utile, mais ne rapporte plus d’UI cette semaine.";
      return fallback;
    };
    ...
    const actions = [
      {
        ...
        cta: "Publier",
        status: (() => {
          const s = buildStatus(publishWeek, WEEKLY_GOALS.booster.publish);
          return {
            ...s,
            helper: buildMissionHelper(publishWeek, createActuDone, s.helper),
          };
        })(),
        reward: buildMissionReward(
          publishWeek,
          createActuDone,
          actuProjected,
          actuGained,
      ...
      },
      {
        ...
        cta: "Demander",
        status: (() => {
          const s = buildStatus(reviewWeek, WEEKLY_GOALS.booster.reviews);
          return {
            ...s,
            helper: buildMissionHelper(
              reviewWeek,
              featureMissionDone,
              s.helper,
            ),
      ...
      },
      {
        ...
        cta: "Envoyer",
        status: (() => {
          const s = buildStatus(promoWeek, WEEKLY_GOALS.booster.promo);
          return {
            ...s,
            helper: buildMissionHelper(promoWeek, featureMissionDone, s.helper),
          };
        })(),
        reward: buildMissionReward(
          promoWeek,
      ...
      metrics: [
        ...
        },
        {
          ...
          channels: [
            ...
            {
              name: "Dernier envoi",
              value: formatLastSend(review.last_sent_at),
            },
          ],
        },
        {
          ...
          channels: [
            ...
            {
              name: "Dernier envoi",
              value: formatLastSend(promo.last_sent_at),
            },
          ],
        },
      ],
        ...
        <div className={b.container}>
          <header className={b.headerRow}>
            <div className={b.titleLine}>
              <span aria-hidden className={b.titleIcon}>
                🚀
              </span>
              <div className={styles.title}>Booster</div>
            </div>
            <div className={b.tagline}>
              Faites décoller votre activité. <strong>3 actions</strong>,
              maintenant.
            </div>
            <div className={b.closeWrap}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <ResponsiveActionButton
                  desktopLabel="Fidéliser"
                  mobileIcon="F"
                  href="/dashboard/fideliser"
                  ariaLabel="Aller vers Fidéliser"
                  title="Fidéliser"
                  className={b.headerBtnFideliser}
                />
                <ResponsiveActionButton
                  desktopLabel="iNr'Send"
                  mobileIcon="✉️"
                  href="/dashboard/mails"
                  ariaLabel="Aller vers iNr'Send"
                  title="Ouvrir iNr'Send"
                  className={b.headerBtnInrSend}
                />
                <ResponsiveActionButton
                  desktopLabel="Fermer"
                  mobileIcon="✕"
          ...
          >
            ...
            <ul style={{ margin: 0, paddingLeft: 18 }}>
              ...
              <li>
                Débloquez vos UI hebdomadaires avec le multiplicateur Turbo UI.
              </li>
            ...
            >
              <strong>
                Toutes vos communications sont accessibles dans iNr'Send.
              </strong>
              <br />
              Les publications réalisées depuis Booster sont retrouvables dans
              iNr'Send / Publications, où elles peuvent être consultées,
          ...
          <details className={[styles.blockCard, b.missionAccordion].join(" ")}>
            <summary className={b.missionSummary}>
              ...
              <div className={b.missionSummaryCenter}>
                Turbo UI ×{data.turbo} · Jusqu’à +{data.missions.totalAvailable}{" "}
                UI
              </div>
              <div className={b.missionSummaryMeta}>
                <div className={b.heroScore}>
                  +{data.missions.totalEarned} UI
                </div>
                <span className={b.missionToggle}>
                  Voir le détail{" "}
                  <span className={b.missionChev} aria-hidden>
                    ▾
                  </span>
                </span>
              </div>
            ...
            <div className={b.missionBody}>
              <div className={b.heroMeta}>
                <span>{data.missions.completedCount}/2 missions validées</span>
                <span>
                  {weeklySummary?.turbo?.connectedCount ?? 0}/
                  {weeklySummary?.turbo?.totalChannels ?? 6} canaux connectés
                </span>
                <span>Gains boostés par le multiplicateur</span>
              </div>
              <div className={b.heroMissionGrid}>
                <MissionPill
                  title="Créer une actu"
                  done={data.missions.createActuDone}
                  reward={data.missions.projectedActu}
                  turbo={data.turbo}
                />
                <MissionPill
                  title="Utiliser Booster / Fidéliser"
                  done={data.missions.featureDone}
                  reward={data.missions.projectedFeature}
                  turbo={data.turbo}
                />
              </div>
            </div>
            ...
            <section className={b.grid3} style={{ marginTop: 8 }}>
              {data.metrics.map((m, idx) => (
                <div key={`${m.title}-${idx}`} className={b.stackCard}>
                  ...
                  <TipAccordion
                    styles={styles}
                    title={data.tips[idx].title}
                    lines={data.tips[idx].lines}
                  />
                </div>
              ))}
            </section>
        ...
        >
          <div
            ...
            style={{
              ...
              border: `1px solid ${publishSummary?.failureCount ? "rgba(251,191,36,0.28)" : "rgba(34,197,94,0.28)"}`,
              background:
          ...
          >
            ...
            <StatusMessage
              variant={publishSummary?.failureCount ? "error" : "success"}
              ...
              >
                {publishSummary.entries.map((entry: any) => (
                  ...
                  >
                    ...
                    {entry.error ? (
                      <div
                        style={{ marginTop: 6, fontSize: 13, color: "#ffb4b4" }}
                      >
                        {entry.error}
                      </div>
                    ) : null}
                    {entry.warning_message ? (
                      <div
                        style={{ marginTop: 6, fontSize: 13, color: "#fde68a" }}
                      >
                        {entry.warning_message}
                      </div>
                    ) : null}
...
function MissionPill({
  ...
  return (
    <div
      className={[b.missionPill, done ? b.missionDone : b.missionTodo].join(
        " ",
      )}
    >
      <div>
        <div className={b.missionTitle}>{title}</div>
        <div className={b.missionSubtitle}>
          {done ? "Mission validée" : `+${reward} UI avec ×${turbo}`}
        </div>
      </div>
      <div
        className={[
          b.missionState,
          done ? b.missionStateDone : b.missionStateTodo,
        ].join(" ")}
      >
        {done ? "Fait" : "À faire"}
      </div>
  ...
  return (
    ...
    >
      ...
      <div className={b.actionTop}>
        ...
        <div className={[b.status, toneClass].join(" ")}>
          <span
            className={[
              b.dot,
              b[
                `dot${status.color.charAt(0).toUpperCase()}${status.color.slice(1)}`
              ],
            ].join(" ")}
            aria-hidden
          />
      ...
      <div className={b.actionCenter}>
        <div className={[styles.moduleDesc, b.actionDesc].join(" ")}>
          {desc}
        </div>
...
function MetricCard({
  ...
  const paddedChannels = isCampaign
    ? channels
    : [
        ...channels,
        ...Array.from(
          { length: Math.max(0, 6 - channels.length) },
          (_, idx) => ({ name: `__empty_${idx}`, value: "", empty: true }),
        ),
      ];
  ...
  return (
    <div className={[styles.blockCard, b.metricCard].join(" ")}>
      ...
      <div className={b.metricLine}>
        <div className={b.metricBubble}>
          {week}/{goal}
        </div>
        <div className={[b.progressState, toneClass].join(" ")}>
          {status.label}
        </div>
      </div>
      <div className={b.progressBar}>
        <div
          className={[b.progressFill, toneClass].join(" ")}
      ...
      <div
        className={[
          b.channelGridCompact,
          isCampaign ? b.channelGridCampaign : "",
        ].join(" ")}
      >
        {paddedChannels.map((c: any) => (
          <div
            key={c.name}
            className={[
              b.channelItemCompact,
              c.empty ? b.channelItemPlaceholder : "",
            ].join(" ")}
