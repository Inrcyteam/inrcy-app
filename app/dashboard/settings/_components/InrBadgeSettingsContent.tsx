...
function saveShareSettings(storageKey: string, settings: ShareSettings) {
  try {
...
async function persistShareSettings(settings: ShareSettings) {
  try {
...
export default function InrBadgeSettingsContent({
  ...
  useEffect(() => {
    ...
    const loadServerSettings = async () => {
      try {
        ...
        const json = await res.json().catch(() => null) as { settings?: unknown } | null;
        if (!res.ok || cancelled) return;
        const serverSettings = normalizeInrBadgeShareSettings(json?.settings);
        setSettings(serverSettings);
        saveShareSettings(storageKey, serverSettings);
      } catch {
  ...
  const updateSetting = (key: ShareKey, value: boolean) => {
    ...
    setSettings(next);
    saveShareSettings(storageKey, next);
    void persistShareSettings(next);
  ...
  return (
    <div style={{ display: "grid", gap: 14 }}>
      ...
      <div style={cardStyle}>
        <h3 style={sectionTitleStyle}>Actions du badge</h3>
        <div style={gridStyle}>
          <FieldToggle label="Prise de RDV" checked={settings.appointment} helper="Option prête pour l'étape publique." onChange={(value) => updateSetting("appointment", value)} />
          <FieldToggle label="Demande de devis" checked={settings.quote} helper="Option prête pour l'étape publique." onChange={(value) => updateSetting("quote", value)} />
        </div>
        <button type="button" onClick={onOpenActivity} style={{ ...smallButtonStyle, marginTop: 12 }}>enrichir avec mon activité</button>
      </div>
...
const heroTitleStyle: React.CSSProperties = { margin: 0, color: "#fff", fontSize: 19 };
const heroTextStyle: React.CSSProperties = { margin: "4px 0 0", color: "rgba(255,255,255,0.88)", fontWeight: 700 };
const heroSubTextStyle: React.CSSProperties = { margin: "6px 0 0", color: "rgba(226,232,240,0.72)", fontSize: 12, lineHeight: 1.45 };
const sectionTitleStyle: React.CSSProperties = { margin: "0 0 10px", color: "#fff", fontSize: 15 };
const mutedStyle: React.CSSProperties = { margin: "0 0 12px", color: "rgba(226,232,240,0.70)", fontSize: 12, overflowWrap: "anywhere" };
const gridStyle: React.CSSProperties = { display: "grid", gap: 10 };
const buttonGridStyle: React.CSSProperties = { display: "flex", flexWrap: "wrap", gap: 8 };
...
const toggleTitleStyle: React.CSSProperties = { display: "block", fontSize: 13, color: "rgba(255,255,255,0.94)" };
const toggleHelperStyle: React.CSSProperties = { display: "block", marginTop: 3, fontSize: 11, color: "rgba(226,232,240,0.62)", lineHeight: 1.35 };
const previewCardStyle: React.CSSProperties = { display: "grid", gap: 7, justifyItems: "center", textAlign: "center", padding: 16, borderRadius: 18, background: "rgba(255,255,255,0.06)", color: "#fff" };
const previewLogoStyle: React.CSSProperties = { width: 62, height: 62, borderRadius: 999, objectFit: "cover", border: "1px solid rgba(255,255,255,0.22)" };
const previewLogoFallbackStyle: React.CSSProperties = { ...heroIconStyle, width: 62, height: 62 };
const previewPillsStyle: React.CSSProperties = { display: "flex", flexWrap: "wrap", gap: 6, justifyContent: "center", marginTop: 6, fontSize: 11, color: "rgba(255,255,255,0.82)" };
const noticeStyle: React.CSSProperties = { position: "sticky", bottom: 10, justifySelf: "center", padding: "9px 12px", borderRadius: 999, background: "rgba(16,185,129,0.18)", border: "1px solid rgba(16,185,129,0.32)", color: "#d1fae5", fontSize: 12, fontWeight: 800 };
