...
import {
  ...
  isDashboardChannelKey,
} from "@/lib/dashboardChannels";
import {
...
function overviewHasConnection(
  key: CubeKey,
  ov: Overview | null | undefined,
): boolean {
  if (!ov) return false;
  if (key === "site_inrcy") {
    const ownership = (ov as any)?.inrcySiteOwnership;
    if (ownership === "sold" || ownership === "rented") return true;
    return (
      !!ov.sources?.site_inrcy?.connected?.ga4 ||
      !!ov.sources?.site_inrcy?.connected?.gsc
    );
  }
  if (key === "site_web") {
    return (
      !!ov.sources?.site_web?.connected?.ga4 ||
      !!ov.sources?.site_web?.connected?.gsc
    );
  }
  if (key === "gmb") return !!ov.sources?.gmb?.connected;
  if (key === "facebook") return !!ov.sources?.facebook?.connected;
  if (key === "instagram") return !!ov.sources?.instagram?.connected;
  return !!ov.sources?.linkedin?.connected;
}

function anyCubeConnected(
  cubes:
    | Partial<Record<CubeKey, Overview | null | undefined>>
    | Record<CubeKey, CubeState>,
): boolean {
  const keys: CubeKey[] = [
    "site_inrcy",
    "site_web",
  ...
  return keys.some((key) => {
    const raw = (cubes as any)?.[key];
    const ov = raw && "ov" in raw ? raw.ov : raw;
    return overviewHasConnection(key, ov);
  });
}

function normalizeCapturedLeads(
  raw: unknown,
  fallback?: CapturedLeads,
): CapturedLeads {
  const value =
    raw && typeof raw === "object" ? (raw as Partial<CapturedLeads>) : {};
  return {
...
export default function StatsClient() {
  ...
  const [dataByCube, setDataByCube] =
    useState<Record<CubeKey, CubeState>>(emptyCubeState);

  const [summaryOpp, setSummaryOpp] = useState<{
    loading: boolean;
    total: number;
  ...
  const [summaryProfile, setSummaryProfile] = useState<{
    ...
    avg_basket: number;
  }>({ lead_conversion_rate: 0, avg_basket: 0 });
  const [summaryEstimatedByCube, setSummaryEstimatedByCube] = useState<
    Record<CubeKey, number>
  >({
    site_inrcy: 0,
    site_web: 0,
    gmb: 0,
  ...
  const [activeStatsPanel, setActiveStatsPanel] =
    useState<StatsPanelKey>("all");
  ...
  useBrowserLayoutEffect(() => {
    const cachedCube = parseCachedCubeSnapshot(
      readUiCacheValue(cubeSessionKey(period)),
    );
    const cachedSummary = parseCachedSummarySnapshot(
      readUiCacheValue(summarySessionKey(period)),
    );
      ...
      setDataByCube((prev) => {
        ...
        for (const k of Object.keys(cachedCube.overviews) as CubeKey[]) {
          next[k] = {
            ...
            loading: false,
            error: undefined,
            capturedLeads: normalizeCapturedLeads(
              cachedCube.blocks?.[k]?.capturedLeads,
              prev[k]?.capturedLeads,
            ),
          };
        }
        return next;
    ...
    if (cachedSummary) {
      ...
      setSummaryProfile({
        lead_conversion_rate: safeNum(
          cachedSummary.profile?.lead_conversion_rate,
        ),
  ...
  const applyBulkPayload = useCallback(
    (targetPeriod: Period, next: BulkFetchResult, syncedAt: number) => {
      const snap = next.overviews as Record<CubeKey, Overview>;
      periodCacheRef.current.set(targetPeriod, snap);
      try {
        writeUiCacheValue(
          cubeSessionKey(targetPeriod),
          JSON.stringify({
            syncedAt,
      ...
      setDataByCube((prev) => {
        ...
        const previousHadConnection = anyCubeConnected(prev);
        const incomingHasConnection = anyCubeConnected(snap);
        const keepCurrentVisibleState =
          previousHadConnection && !incomingHasConnection;

        for (const k of Object.keys(snap) as CubeKey[]) {
          const incomingOverview = snap[k] ?? null;
          const previousOverview = prev[k]?.ov ?? null;
          const safeOverview =
            keepCurrentVisibleState && previousOverview
              ? previousOverview
              : incomingOverview;
  ...
  const applyChannelRefreshPayload = useCallback(
    (
      channel: DashboardChannelKey,
      payload: ChannelRefreshResponse | null | undefined,
      fallbackSyncAt?: number,
    ) => {
      const syncAt = Number.isFinite(Number(fallbackSyncAt))
      ...
      for (const targetPeriod of [7, 30] as const) {
        ...
        const periodSyncAt = Number.isFinite(Number(periodPayload?.syncedAt))
          ? Number(periodPayload?.syncedAt)
          : (block.syncAt ?? syncAt);
        ...
        mergeChannelBlockIntoCachedSnapshots({
          ...
          snapshotDate:
            typeof periodPayload?.snapshotDate === "string"
              ? periodPayload.snapshotDate
        ...
        setDataByCube((prev) => ({
          ...
          [channel]: {
            ov:
              (periodPayload?.overview as Overview | undefined) ??
              (block.overview as Overview | null | undefined) ??
              prev[channel]?.ov ??
              null,
            loading: false,
            ...
            capturedLeads: normalizeCapturedLeads(
              block.capturedLeads,
              prev[channel]?.capturedLeads,
            ),
          },
        }));

        const cachedSummary = parseCachedSummarySnapshot(
          readUiCacheValue(summarySessionKey(targetPeriod)),
        );
        if (cachedSummary) {
          setSummaryHydrated(true);
          setSummaryOpp({
  ...
  const refreshChannelFromApi = useCallback(
    async (channel: DashboardChannelKey, fallbackSyncAt?: number) => {
      ...
      const json = (await res
        .json()
        .catch(() => null)) as ChannelRefreshResponse | null;
  ...
  const applyBootstrapPayload = useCallback(
    (bootstrap: DailyStatsRefreshBootstrapResponse) => {
      const syncAt = Number.isFinite(Number(bootstrap?.syncAt))
        ? Number(bootstrap.syncAt)
        : Date.now();
      const bootstrapSnapshotDate =
        typeof bootstrap?.snapshotDate === "string"
          ? bootstrap.snapshotDate
          : expectedUiSnapshotDate();

      markDailyStatsRefreshBootstrapChecked({
        snapshotDate: bootstrapSnapshotDate,
        checkedAt: Date.now(),
        syncAt,
      });
      ...
      if (generator) {
        ...
        try {
          const generatorSnapshotDate =
            typeof generator?.meta?.snapshotDate === "string"
          ...
          writeUiCacheValue(
            ...
            JSON.stringify({
              syncedAt: syncAt,
              snapshotDate: generatorSnapshotDate,
              payload: generator,
            }),
          );
        } catch {
      ...
      for (const [periodKey, rawPayload] of Object.entries(
        bootstrap.inrstats || {},
      )) {
        const payload = rawPayload as any;
        const targetPeriod = Number(periodKey) as Period;
        const overviews = (payload?.overviews || {}) as Partial<
          Record<CubeKey, Overview>
        >;
        const payloadSnapshotDate =
          typeof payload?.meta?.snapshotDate === "string"
            ? payload.meta.snapshotDate
            : getOverviewSnapshotDate(overviews) ||
              bootstrapSnapshotDate ||
              null;
        const next: BulkFetchResult = {
          ...
          profile: {
            lead_conversion_rate: safeNum(
              payload?.profile?.lead_conversion_rate,
            ),
  ...
  const syncFromServerCacheIfNeeded = useCallback(
    async (force = false) => {
      ...
      if (!force) {
        ...
        if (wasServerCacheSyncCheckedRecently("stats", { snapshotDate }))
          return;
      }
      ...
      const job = (async () => {
        ...
        try {
          ...
          if (json?.connections?.needsRefresh === true) {
            ...
            markServerCacheSyncChecked("stats", {
              snapshotDate,
              checkedAt: Date.now(),
          ...
          const periodStatuses: Partial<
            Record<
              Period,
              {
                syncedAt?: number;
                channels?: Partial<Record<DashboardChannelKey, number>>;
              }
            >
          > = {
          ...
          const staleChannelsByPeriod = ([7, 30] as Period[]).reduce(
            (acc, days) => {
                  ...
                  : Object.entries(channels)
                      .filter(
                        ([channel, serverTs]) =>
                          Number(serverTs ?? 0) >
                          readCachedChannelSyncAt(
                            days as StatsWarmPeriod,
                            channel as DashboardChannelKey,
                          ),
                      )
          ...
          const periodsToRefresh = ([7, 30] as Period[])
            ...
            .filter(
              (item) =>
                item.syncedAt > getLocalPeriodSyncAt(item.days) &&
                (getLocalPeriodSyncAt(item.days) === 0 ||
                  item.staleChannels.length === 0),
            );
          const staleChannels = Array.from(
            new Set(
              ([7, 30] as Period[])
                .filter(
                  (days) =>
                    !periodsToRefresh.some((item) => item.days === days),
                )
  ...
  const hydrateFromSessionCache = useCallback((targetPeriod: Period) => {
    ...
    const cachedCube = parseCachedCubeSnapshot(
      readUiCacheValue(cubeSessionKey(targetPeriod)),
    );
    const cachedSummary = parseCachedSummarySnapshot(
      readUiCacheValue(summarySessionKey(targetPeriod)),
    );
  ...
  const fetchBulkStats = async (
    ...
    const r = await fetch(`/api/stats/dashboard-bulk?${params.toString()}`, {
      cache: "no-store",
    });
    if (!r.ok) {
    ...
    const overviews = (json?.overviews || {}) as Partial<
      Record<CubeKey, Overview>
    >;
    const byCubePartial = json?.opportunities?.byCube || {};
    const snapshotDate =
      typeof json?.meta?.snapshotDate === "string"
        ? json.meta.snapshotDate
        : getOverviewSnapshotDate(overviews) || expectedSnapshotDate;
    return {
      overviews,
      summary: {
  ...
  useEffect(() => {
    ...
    if (
      hasFreshLocalStats &&
      wasDailyStatsRefreshBootstrapCheckedRecently({ snapshotDate })
    ) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const cachedCubeSnapshot = parseCachedCubeSnapshot(
        readUiCacheValue(cubeSessionKey(period)),
      );
      const lastChannelSyncAt = getStatsLastChannelSyncAt();
      const cachedSummary = parseCachedSummarySnapshot(
      ...
      const hasFreshCachedSummary =
        !!cachedSummary &&
        cachedSummary.syncedAt >= lastChannelSyncAt &&
        cachedSummary.snapshotDate === expectedUiSnapshotDate();
      const hasFreshCapturedLeads = hasCapturedLeadsBlocks(
        cachedCubeSnapshot?.blocks,
      );
        ...
        setDataByCube((prev) => {
          ...
          for (const k of Object.keys(cached) as CubeKey[]) {
            next[k] = {
              ...
              error: undefined,
              capturedLeads: normalizeCapturedLeads(
                cachedCubeSnapshot?.blocks?.[k]?.capturedLeads,
                prev[k]?.capturedLeads,
              ),
            };
          }
      ...
      setDataByCube((prev) => {
        const next: any = { ...prev };
        for (const k of keys) {
          // Ne jamais repasser visuellement un canal déjà connu en état neutre/déconnecté
          // pendant un refresh automatique : on garde l'état affiché jusqu'au payload final.
          next[k] = { ...next[k], loading: !next[k]?.ov, error: undefined };
        }
        return next;
      });
      setSummaryOpp((prev) => ({
        ...
        loading:
          prev.total <= 0 &&
          !Object.values(prev.byCube || {}).some((value) => safeNum(value) > 0),
      }));
        ...
        setDataByCube((prev) => {
          ...
          for (const k of keys) {
            updated[k] = {
              ...updated[k],
              loading: false,
              error: updated[k]?.ov ? undefined : msg,
            };
          }
          return updated;
        });
  ...
  useEffect(() => {
    ...
    const handleChannelUpdated = (event: Event) => {
      const detail = (event as CustomEvent<{ channel?: DashboardChannelKey }>)
        .detail;
    ...
    window.addEventListener(
      "inrcy:channel-updated",
      handleChannelUpdated as EventListener,
    );
    window.addEventListener(
      "inrcy:channels-updated",
      handleChannelsUpdated as EventListener,
    );
    return () => {
  ...
  const computedEstimatedByCube = useMemo<Record<CubeKey, number>>(() => {
    const rate =
      Math.max(0, safeNum(summaryProfile.lead_conversion_rate)) / 100;
    const basket = Math.max(0, safeNum(summaryProfile.avg_basket));
    const estimate = (opportunities: number) =>
      Math.round(Math.max(0, safeNum(opportunities)) * rate * basket);
    ...
    centralByCube,
    summaryProfile.avg_basket,
    summaryProfile.lead_conversion_rate,
  ]);
  ...
  const totalCapturedLeads30 = useMemo(() => {
    return models.reduce(
      (total, model) => total + safeNum(model.capturedLeads.month),
      0,
    );
  }, [models]);
  ...
  const activeModel =
    activeStatsPanel === "all"
      ? null
      : (models.find((model) => model.key === activeStatsPanel) ??
        models[0] ??
        null);
  ...
  return (
    <div className={styles.page}>
      <div className={styles.header}>
        <div className={styles.headerTop}>
          <div className={styles.brand}>
            <Image
              ...
              priority
            />
            <div className={`${styles.tagline} ${styles.taglineDesktop}`}>
              Vos données analysées en mode business.
            </div>
          </div>
          ...
          <div className={styles.headerActions}>
            <div className={styles.headerCloseControls}>
              ...
              <ResponsiveActionButton
                ...
                title={
                  lastRefreshAt
                    ? `Dernière actualisation : ${new Date(lastRefreshAt).toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit", second: "2-digit" })}`
                    : "Actualiser les données iNrStats"
                }
              />
              <ResponsiveActionButton
                desktopLabel="Fermer"
        ...
        <div className={`${styles.tagline} ${styles.taglineMobile}`}>
          Vos données analysées en mode business.
        </div>
      </div>
      ...
      >
        <p style={{ marginTop: 0 }}>
          iNr’Stats analyse les données récupérées sur vos canaux (site, Google,
          réseaux…) et les transforme en analyse business.
        </p>
        <ul style={{ margin: 0, paddingLeft: 18 }}>
          <li>
            Comprenez votre potentiel d’opportunités sur les 30 jours à venir.
          </li>
          <li>Identifiez les actions à mener pour capter ce potentiel.</li>
          <li>
            Suivez l’évolution par canal et identifiez les actions à mener sur
            les 30 jours à venir.
          </li>
        </ul>
      </HelpModal>
      ...
      {statsMenuOpen ? (
        <div
          className={styles.statsMobileDrawerOverlay}
          role="presentation"
          onClick={() => setStatsMenuOpen(false)}
        >
          <aside
            className={styles.statsMobileDrawer}
            aria-label="Choisir une vue iNrStats"
          ...
          >
            <div className={styles.statsMobileDrawerHead}>
              ...
              <button
                type="button"
                onClick={() => setStatsMenuOpen(false)}
                aria-label="Fermer le menu des canaux"
              >
                ×
              </button>
            </div>
            ...
            <div className={styles.statsMobileDrawerList}>
              ...
              >
                ...
                <span className={styles.statsRailValue}>
                  +{fmtInt(centralPotential30)}
                </span>
              </button>

              {models.map((model) => {
                const isSite =
                  model.key === "site_inrcy" || model.key === "site_web";
                const connected = isSite
                  ? !!model.connections.ga4 || !!model.connections.gsc
                  : !!model.connections.main;
      ...
      <div className={styles.statsWorkspace}>
        ...
        <main className={styles.statsPanel}>
          {activeStatsPanel === "all" ? (
            <section
              className={styles.allStatsPanel}
              aria-label="Vue globale iNrStats"
            >
              <div className={styles.allStatsHero}>
                <div>
                  <div className={styles.allStatsEyebrow}>Vue globale</div>
                  <h2 className={styles.allStatsTitle}>
                    Tous vos canaux en un coup d’œil
                  </h2>
                  <p className={styles.allStatsText}>
                    Synthèse par canal : opportunités activables, CA potentiel
                    et outil recommandé.
                  </p>
                </div>
                ...
                <div className={styles.allStatsKpis}>
                  <div
                    className={`${styles.allStatsKpi} ${styles.kpiToneBlue}`}
                  >
                    <span>Opportunités</span>
                    <b>+{fmtInt(centralPotential30)}</b>
                  </div>
                  <div
                    className={`${styles.allStatsKpi} ${styles.kpiTonePurple}`}
                  >
                    <span>CA potentiel</span>
                    <b>
                      +
                      {fmtInt(
                        summaryActionItems.reduce(
                          (total, item) => total + safeNum(item.revenue),
                          0,
                        ),
                      )}{" "}
                      €
                  ...
                  <div
                    className={`${styles.allStatsKpi} ${styles.kpiToneGreen}`}
                  >
                    <span>Demandes captées 30 j</span>
                    <b>{fmtInt(totalCapturedLeads30)}</b>
                  </div>
                  <div
                    className={`${styles.allStatsKpi} ${styles.kpiToneSlate}`}
                  >
                    <span>Canaux</span>
                    <b>{models.length}</b>
                  </div>
              ...
              <div className={styles.allStatsActions}>
                {models.map((model) => {
                  ...
                  const revenue =
                    summaryEstimatedByCube[model.key] ||
                    computedEstimatedByCube[model.key] ||
                    actionItem?.revenue ||
                    0;
                  ...
                  const channelText =
                    model.insights.find(
                      (text) =>
                        !text.toLowerCase().startsWith("recommandation"),
                    ) ||
                    model.capturedLeadsHint ||
                    model.subtitle;
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        type="button"
                        className={styles.allStatsChannelButton}
                        onClick={() => scrollTo(model.key)}
                      >
                        <span className={styles.allStatsChannelName}>
                          {model.title}
                        </span>
                      </button>
                      ...
                      <div className={styles.allStatsRecommendedAction}>
                        <span
                          className={`${styles.allStatsToolBadge} ${connected ? "" : styles.allStatsToolBadgeConnect}`}
                        >
            ...
            <section
              className={styles.channelStatsPanel}
              aria-label={`Données ${activeModel.title}`}
            >
              <div className={styles.channelStatsHeader}>
                ...
                <div
                  className={`${styles.allStatsKpis} ${styles.channelStatsKpis}`}
                >
                  <div
                    className={`${styles.allStatsKpi} ${styles.kpiToneBlue}`}
                  >
                  ...
                  >
                    ...
                    <b>
                      +
                      {fmtInt(
                        summaryEstimatedByCube[activeModel.key] ||
                          computedEstimatedByCube[activeModel.key] ||
                          0,
                      )}{" "}
                      €
                    </b>
                  ...
                  <div
                    className={`${styles.allStatsKpi} ${styles.kpiToneGreen} ${styles.channelDemandesKpi}`}
                  >
                    <span className={styles.channelDemandesKpiLabel}>
                      Demandes captées 7j / 30j
                    </span>
                    <b>
                      {activeModel.capturedLeadsUnavailable
                        ? "—"
                        : `${fmtInt(activeModel.capturedLeads.week)} / ${fmtInt(activeModel.capturedLeads.month)}`}
                    </b>
                  </div>
                </div>
              </div>
