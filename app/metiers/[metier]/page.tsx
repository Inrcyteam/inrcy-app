...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!companies.length) return { title: "Métier introuvable | iNr'Search", robots: { index: false, follow: false } };
  ...
  const description = `Découvrez les entreprises ${label.toLowerCase()} présentes sur iNr'Search.`;
  return {
    title,
    description,
  ...
  return (
    <main className={styles.page}>
      ...
      <header className={styles.header}>
        <div className={styles.breadcrumbs}><Link href="/entreprises">Entreprises</Link><span>›</span><Link href="/metiers">Métiers</Link><span>›</span><span>{label}</span></div>
        <span className={styles.kicker}>Professionnels</span>
        <h1>{label}</h1>
        <p>Découvrez les entreprises et professionnels spécialisés en {label.toLowerCase()}.</p>
      </header>
      {cities.length ? (
        <nav className={styles.filterRow} aria-label="Villes">
