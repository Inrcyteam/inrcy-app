...
const MAX_INPUT_BYTES = INR_MEDIA_VIDEO_SOURCE_MAX_BYTES;
...
async function downloadSourceVideo(source: BoosterVideoTransformSource) {
  ...
  if (storagePath) {
    ...
    if (error || !data) {
      throw new Error(
        error?.message ||
          "Impossible de lire la vidéo source depuis le stockage.",
      );
    }
  ...
  if (!publicUrl || !/^https?:\/\//i.test(publicUrl)) {
    throw new Error(
      "Vidéo source manquante : storagePath ou URL publique requis.",
    );
  }
  ...
  if (!res.ok) {
    throw new Error(
      `Impossible de télécharger la vidéo source (${res.status}).`,
    );
  }
...
async function makeFfmpegExecutableIfNeeded(ffmpegPath: string) {
  if (!ffmpegPath || ffmpegPath === "ffmpeg" || process.platform === "win32")
    return;
  try {
...
async function ensureFfmpegAvailable() {
  ...
  throw new Error(
    "Adaptation automatique indisponible : FFmpeg n’est pas exécutable sur le serveur.",
  );
}
...
async function runFfmpegVariant(
  ...
  try {
    ...
    throw new Error(
      `Transformation vidéo échouée (${plan.target.label}). ${details}`.trim(),
    );
  }
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const variants = Array.isArray(body.variants)
      ? body.variants.slice(0, MAX_VARIANTS_PER_REQUEST)
      : [];
    ...
    if (sourceBuffer.length > MAX_INPUT_BYTES) {
      return NextResponse.json(
        {
          error:
            "Vidéo source trop lourde pour la transformation locale. Limite actuelle : 100 Mo.",
        },
        { status: 413 },
      );
    }
    ...
    const inputPath = path.join(
      tempDir,
      `source.${getSourceExtension(source)}`,
    );
    await writeFile(inputPath, sourceBuffer);
    const fallbackDuration =
      typeof source.duration === "number"
        ? source.duration
        : (source.sourceMetadata?.duration ?? null);
    const duration =
      fallbackDuration || (await probeDurationSeconds(inputPath));
