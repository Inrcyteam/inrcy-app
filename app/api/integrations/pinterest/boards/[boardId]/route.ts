...
import { deletePinterestBoard, getPinterestAccessToken, updatePinterestBoard } from "@/lib/pinterestOAuth";
...
async function resolvePinterestContext(request: Request) {
  ...
  if (authErr || !user) return { error: NextResponse.json({ ok: false, error: "Non authentifié." }, { status: 401 }) };
...
export async function PATCH(request: Request, context: { params: Promise<{ boardId: string }> }) {
  try {
    ...
    if (!cleanBoardId) return NextResponse.json({ ok: false, error: "Tableau Pinterest invalide." }, { status: 400 });
    ...
    const board = await updatePinterestBoard(resolved.accessToken, cleanBoardId, name);
    return NextResponse.json({ ok: true, board });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Modification du tableau Pinterest impossible.";
...
export async function DELETE(request: Request, context: { params: Promise<{ boardId: string }> }) {
  try {
    ...
    const message = error instanceof Error ? error.message : "Suppression du tableau Pinterest impossible.";
    return NextResponse.json({ ok: false, error: message }, { status: 400 });
  }
}
