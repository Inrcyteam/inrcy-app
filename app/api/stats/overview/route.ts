...
export async function GET(request: Request) {
  try {
    ...
    function isChannelConnected(provider: "facebook" | "instagram" | "linkedin" | "gmb", row: Record<string, unknown>) {
      const status = String(row["status"] ?? "");
      const resourceId = String(row["resource_id"] ?? "").trim();
      const hasToken = !!String(row["access_token_enc"] ?? "").trim();
      const expired = isExpired(row["expires_at"]);

      if (provider === "facebook") {
        return status === "connected" && !!resourceId && hasToken && !expired;
      }
      if (provider === "instagram") {
        return status === "connected" && !!resourceId && hasToken && !expired;
      }
      if (provider === "linkedin") {
        return status === "connected" && hasToken && !expired;
      }
      return status === "connected" && !!resourceId && !expired;
    }
    ...
    async function fetchSocialStatus() {
      ...
      try {
        out.facebook.connected = isChannelConnected("facebook", latestIntegrationAny("facebook", "facebook", "facebook"));
      } catch {}

      try {
        out.instagram.connected = isChannelConnected("instagram", latestIntegrationAny("instagram", "instagram", "instagram"));
      } catch {}

      try {
        out.linkedin.connected = isChannelConnected("linkedin", latestIntegrationAny("linkedin", "linkedin", "linkedin"));
      } catch {}

      try {
        out.gmb.connected = isChannelConnected("gmb", latestIntegrationAny("google", "gmb", "gmb"));
      } catch {}
...
const sources: Array<{ key: StatsSourceKey; ga4Property?: string; gscProperty?: string }> = [
  ...
  },
  {
    ...
    try {
      ...
      sourcesStatus.facebook.connected = isChannelConnected("facebook", fbRow);
    ...
    try {
      ...
      sourcesStatus.instagram.connected = isChannelConnected("instagram", igRow);
...
// LinkedIn: connected if an OAuth row exists.
    try {
      ...
      sourcesStatus.linkedin.connected = isChannelConnected("linkedin", liRow);
...
// GMB: the UI needs a stable "connected" flag like GA4/GSC.
    ...
    try {
      ...
      const resourceId = String(gmbRow["resource_id"] || "");
      sourcesStatus.gmb.connected = isChannelConnected("gmb", gmbRow);
